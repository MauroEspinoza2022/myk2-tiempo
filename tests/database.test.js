...
await db.exec(await readFile(new URL('../supabase/005_company_roles.sql',import.meta.url),'utf8'));
...
const company='aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa';await db.query(`insert into public.companies(id,name) values ($1,'Empresa A')`,[company]);
await db.query(`insert into auth.users(id,email,email_confirmed_at,raw_user_meta_data) values ($1,'a@example.test',now(),$3),($2,'mespinozahse@gmail.com',now(),$4)`,[a,b,JSON.stringify({first_name:'Prueba',last_name:'A',company_id:company,document_type:'DNI',document_number:'12345678',phone:'975721020'}),JSON.stringify({first_name:'Mauro',last_name:'Espinoza',document_type:'DNI',document_number:'87654321',phone:'975721021'})]);
...
await db.exec(`set request.jwt.claim.sub='${b}';`);assert.equal((await db.query('select * from public.entries')).rows.length,1);assert.equal((await db.query('select * from public.profiles')).rows.length,2);
await db.exec('reset role;');await db.query('insert into public.admins values ($1)',[b]);await db.exec(`set role authenticated;`);assert.equal((await db.query('select * from public.entries')).rows.length,1);assert.equal((await db.query('select * from public.profiles')).rows.length,2);assert.equal((await db.query('select * from public.audit_log')).rows.length,2);assert.equal((await db.query('select public.access_context() as c')).rows[0].c.role,'developer');

...
const stats=(await db.query('select public.admin_dashboard() as stats')).rows[0].stats;assert.equal(stats.registered,2);assert.equal(stats.sessions30,1);assert.equal(stats.earned,240);assert.equal(stats.directory.find(u=>u.id===a).email,'a@example.test');
